...
import { variantNames, loadVariant, saveVariant, resolveVariant, nearestStoreDir } from "../src/variants.js";

const freshEnv = () => ({ AMC_HOME: fs.mkdtempSync(path.join(os.tmpdir(), "amc-variants-")) });
// neutral cwd: tmp-based, so ancestors can never contain a real .ai-model-configure/variants
...
test("shape-broken variant docs load as null (hand-edit protection)", () => {
  ...
  fs.writeFileSync(path.join(env.AMC_HOME, "variants", "nullrole.json"), JSON.stringify({ roles: { plan: null } }));
...
test("tree-scoped variants shadow the global store; resolution walks up", () => {
  ...
  fs.writeFileSync(path.join(tree, ".ai-model-configure", "variants", "hybrid.json"), "{ broken");
